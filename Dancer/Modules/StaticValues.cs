using System;
using System.Collections.Generic;
using System.Text;

namespace Dancer.Modules
{
    public class StaticValues
    {
        public const float jab1DamageCoefficient = 3f;
        public const float jab2DamageCoefficient = 3f;
        public const float dashAttackDamageCoefficient = 3.5f;
        public const float downTiltDamageCoefficient = 3f;
        public const float downAirDamageCoefficient = 2.1f;
        public const float forwardAirDamageCoefficient = 3.25f;
        public const float upAir1DamageCoefficient = 2.5f;
        public const float upAir2DamageCoefficient = 3.25f;

        public const float secondaryDamageCoefficient = 3f;

        public const float dragonLungeDamageCoefficient = 6f;

        public const float ribbonDuration = 8f;
        public const float ribbonDamageCoefficient = 1f;

    }
}
