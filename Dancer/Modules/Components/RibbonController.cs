    ...
    public class RibbonController : NetworkBehaviour
    {
        ...
        public void StartRibbon()
        {
            ...
            if(RibbonController.naturalSpread)
            {
                ...
                {
                    this.SpeedUpRibbon(Modules.StaticValues.ribbonChainTime);
                    this.spreadsRemaining -= 1;
                }
            }
            else
            {
                this.attachTime = Modules.StaticValues.ribbonChainTime;              
                if (this.spreadsRemaining > 0)
                {
                    this.SearchNewTarget();
                    this.spreadsRemaining -= 1;
        ...
        private void AttachRibbon()
        {
            if (NetworkServer.active)
            {
                ...
                if (this.inflictorRoot && this.nextHealthComponent)
                {
                    ...
                    CharacterBody body = this.nextHealthComponent.body;
                    Vector3 point = this.ownerRoot ? this.ownerRoot.transform.position : base.transform.position;
                    Vector3 launchVector = (point - this.nextRoot.transform.position).normalized;
                    launchVector *= Modules.StaticValues.ribbonPullForce;

                    if (this.nextRoot.GetComponent<KinematicCharacterController.KinematicCharacterMotor>())
                    {
                        this.nextRoot.GetComponent<KinematicCharacterController.KinematicCharacterMotor>().ForceUnground();
                    }
        ...
        {
            float percentCompleted = this.fixedAttachStopwatch / this.attachTime;
            this.fixedAttachStopwatch = newTime * percentCompleted;
            percentCompleted = this.attachStopwatch / this.attachTime;
            this.attachStopwatch = newTime * percentCompleted;

            this.attachTime = newTime;
        ...
        public float attachTime = Modules.StaticValues.ribbonNaturalChainTime;
