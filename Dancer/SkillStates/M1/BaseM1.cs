...
namespace Dancer.SkillStates.M1
{
    ...
    public class BaseM1 : BaseSkillState
    {
        ...
        public virtual void PlayHitSound()
        {
            ...
            {
                Util.PlaySound(critHitSoundString, gameObject);
            }
            else
            {
        ...
        private void FireAttack()
        {
            ...
            for (int i = 0; i < array.Length; i++)
            {
                ...
                if ((bool)healthComponent2)
                {
                    ...
                    {
                        healthComponent2.body.characterMotor.velocity.y = 0f;
                    }
        ...
        public override void FixedUpdate()
        {
            base.FixedUpdate();
            bool flag = true;
            if (cancelledFromSprinting && (characterBody.isSprinting || inputBank.sprint.justPressed) && inputBank.skill1.down)
            {
                outer.SetNextStateToMain();
                cancelled = true;
                return;
            }
            if (!isGrounded && isDash && inputBank.jump.down)
            {
                jumpCancelled = true;
            }
            if (jumpCancelled && fixedAge >= duration * (attackStartTime + attackEndTime) / 2f && !isGrounded)
            {
                cancelled = true;
                SetNextStateFromJump();
                return;
            }
            ...
            if (stopwatch >= duration - earlyExitTime && isCombo)
            {
                ...
                if ((bool)component && component.state is GenericCharacterMain && inputBank.skill1.down)
                {
                    if (!hasFired)
                    {
                        FireAttack();
